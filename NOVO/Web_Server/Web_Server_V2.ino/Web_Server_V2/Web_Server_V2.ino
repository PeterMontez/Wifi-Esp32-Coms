...
void moveServos()
{
  String tempServos = servosFull;
  String valores[4];
  int counter = 0;

  while (tempServos.length() > 0)
  {
    int index = tempServos.indexOf(';');
    if (index == -1)
    {
      valores[counter++] = tempServos;
      break;
    }
    else
    {
      valores[counter++] = tempServos.substring(0, index);
      tempServos = tempServos.substring(index + 1);
    }
  }

  if (valores[0].toInt() != servo1)
  {
    servo1 = valores[0].toInt();
    Garra.write(servo1);
  }
  if (valores[1].toInt() != servo2)
  {
    servo2 = valores[1].toInt();
    Pulso.write(servo2);
  }
  if (valores[2].toInt() != servo3)
  {
    servo3 = valores[2].toInt();
    Cutuvelo.write(servo3);
  }
  if (valores[3].toInt() != servo4)
  {
    servo4 = valores[3].toInt();
    Base.write(servo4);
  }
